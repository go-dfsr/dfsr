...
func (c *cacher) Vector(group ole.GUID) (vector *versionvector.Vector, err error) {
	return c.vc.Lookup(group)
}

func (c *cacher) Backlog(vector *versionvector.Vector) (backlog []int, err error) {
	return c.r.Backlog(vector)
}

func (c *cacher) Report(group *ole.GUID, vector *versionvector.Vector, backlog, files bool) (data *ole.SafeArrayConversion, report string, err error) {
